...
class RentalList extends Component {
  constructor(props) {
    ...
    const audioRentals =
      props.rentals &&
      props.rentals.filter((audioRentals) => audioRentals.category === "audio");
    const videoRentals =
      props.rentals &&
      props.rentals.filter((videoRentals) => videoRentals.category === "video");
    const lightsRentals =
      props.rentals &&
      props.rentals.filter(
        (lightsRentals) => lightsRentals.category === "lights"
      );
  ...
  renderAudioRentals() {
    return (
      this.state.audioRentals &&
      this.state.audioRentals.map((rental) => {
        return <RentalCard key={rental._id} rental={rental} />;
      })
    );
  }
  renderVideoRentals() {
    return (
      this.state.videoRentals &&
      this.state.videoRentals.map((rental) => {
        return <RentalCard key={rental._id} rental={rental} />;
      })
    );
  }
  renderLightsRentals() {
    return (
      this.state.lightsRentals &&
      this.state.lightsRentals.map((rental) => {
        return <RentalCard key={rental._id} rental={rental} />;
      })
    );
  }
